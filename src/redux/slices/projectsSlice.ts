...
export const projectsSlice = createSlice({
    ...
    reducers: {
        ...
        addTask: (state, action: PayloadAction<{ projectId: string | undefined; task: TaskInterface }>) => {
            state.projects.map(project =>
                project.id === action.payload.projectId
                    ? project.columns['queue'].tasks.push(action.payload.task)
                    : project
            );

            state.projects.map(project =>
                project.id === action.payload.projectId ? (project.status = 'progress') : project
            );
        ...
        deleteTask: (
            state,
            action: PayloadAction<{ projectId: string | undefined; columnId: string; taskId: string }>
        ) => {
            ...
            currentProject.columns[columnId].tasks = currentProject.columns[columnId].tasks.filter(
                task => task.id !== taskId
            );
            ...
            if (currentProject.columns['done'].tasks.length) {
                if (
                    currentProject.columns['queue'].tasks.length === 0 &&
                    currentProject.columns['development'].tasks.length === 0
                ) {
        ...
        updateTaskStatus: (
            ...
            if (
                currentProject.columns['done'].tasks.length &&
                currentProject.columns['queue'].tasks.length === 0 &&
                currentProject.columns['development'].tasks.length === 0
            ) {
                currentProject.status = 'done';
